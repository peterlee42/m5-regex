...
public class Main {
    ...
    // Modify this code to check if the given string is non-empty, contains at
    // least
    ...
    public static boolean checkForPassword(String str, int minLength) {
        ...
        final boolean length = str.length() >= minLength;
        ...
        final boolean propertyOne = lowercase && uppercase && digit && length;
        return propertyOne;
    }

    // Method 2 for checking if a string conforms to a regex: using Matcher.find
    // Modify this method to return a list of all email addresses contained in
    // the
    ...
    // Modify this method to check whether or not the string contains the same
    // capital letter
